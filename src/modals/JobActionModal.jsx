...
// EditView sub-component remains unchanged
function EditView({ job, onCancel, onUpdate, isGoogleMapsLoaded }) {
    // ... all the code for EditView is the same
...
// The main modal component now uses the currentUserId to determine if the user is the owner
export default function JobActionModal({ job, onClose, onUpdate, onDelete, isGoogleMapsLoaded, currentUserId }) {
  const [mode, setMode] = useState('view');
  const handleDelete = () => {
    onDelete(job.id);
  };

  // Check if the currently logged-in user is the one who created the job
  const isOwner = currentUserId === job.client_id;

  const detailItem = (label, value, isCurrency = false) => (
    // ... detailItem function is unchanged
    <div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === 'edit' && isOwner ? ( // Only allow editing if the user is the owner
          <>
          ...
          <>
            ...
            {mode === 'view' && (
              <div className="flex justify-end gap-4">
                ...
                {/* Only show Edit and Delete buttons if the user is the owner */}
                {isOwner && (
                  <>
